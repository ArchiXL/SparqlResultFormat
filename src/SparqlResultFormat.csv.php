...
class SparqlResultFormatCSV extends SparqlResultFormatBase implements SparqlFormat {
	...
	function generateHtmlContainerCode( $options ) {
		...
		$containerHtml = "<div id='$divId-wrapper'>";
		if ( empty( $linkButtonLabel ) ) {
			$containerHtml .= "<a id='$divId-click'>$label</a>";
		} else {
			$containerHtml .= "<button id='$divId-click' class='$linkButtonCSSClass' type='button'>$linkButtonLabel</button>";
		}
		$containerHtml .= "<span id='$divId-loader' class='loader' style='display:none;'><img src='$wgScriptPath/extensions/SparqlResultFormat/img/spinner.gif'/></span><span id='$divId-error-status' class='csv-error-status' style='display:none;'><i class='fas fa-exclamation-triangle' style='color: #e60606d1;'></i></span></div>";

		$htmlContainer = "<div id='$divId' style='$divStyle' sparql-query='$escapedQuery' class='sparql-to-csv'>$containerHtml</div>";
