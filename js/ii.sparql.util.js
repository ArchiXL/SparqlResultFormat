spqlib.util = ( function ( sparqljs ) {
	 ...
	 * Uses MediaWiki API module for SPARQL queries
	 */
	my.doQuery = function query( queryEndpoint, targetEndpointName, sparql, successCallback,
		...
		var queryTimeout = configuration.queryTimeout || 31000,
			api,
			idContainer,
			progressBar;
		...
		api.get( {
			...
			if ( response.sparqlresult ) {
				successCallback( response.sparqlresult, configuration, caller );
			} else if ( response.error ) {
				if ( failCallback && typeof failCallback === 'function' ) {
					failCallback( configuration, null, response.error.info || 'Unknown error' );
				}
			}
		} ).fail( function ( code, result ) {
			var textStatus = code;
			if ( code === 'http' && result && result.textStatus === 'timeout' ) {
				...
				textStatus = result.error.info;
			}
			if ( failCallback && typeof failCallback === 'function' ) {
				failCallback( configuration, result, textStatus );
			}
		} );
	};
