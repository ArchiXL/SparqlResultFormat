	...
	*/
	...
	my.exportAsImage = function () {
		var id = $( this ).attr( 'sprf-graph-id' ),
			png = spqlib.graph.graphImpl().exportAsImage( id ),
			filename = id + '-export.png';

		// Convert base64 data URL to blob and trigger download
		var data = png.split( ',' ),
			byteString = atob( data[ 1 ] ),
			mimeString = data[ 0 ].split( ':' )[ 1 ].split( ';' )[ 0 ],
